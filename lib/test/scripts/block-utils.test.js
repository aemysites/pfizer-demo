...
describe('Utils methods', () => {
  before(async () => {
    ...
    blockUtils = await import('../../scripts/lib-franklin/lib-franklin.js');
  ...
  it('Loads CSS', async () => {
    ...
    const load = await new Promise((resolve) => {
      blockUtils.loadCSS('/lib/test/scripts/test.css', (e) => resolve(e));
    });
    ...
    // shows promise error in console
    const error = await new Promise((resolve) => {
      blockUtils.loadCSS('/lib/test/scripts/nope.css', resolve);
    });
  ...
  it('Creates optimized picture', async () => {
    const $picture = blockUtils.createOptimizedPicture('/lib/test/scripts/media_mock.png');
...
describe('Sections and blocks', () => {
  ...
  it('Loads blocks', async () => {
    ...
    await waitUntil(() => document.querySelector('main .block[data-block-status="activated"]'));
    document.querySelectorAll('main .block').forEach(($block) => {
      expect($block.dataset.blockStatus).to.equal('activated');
    });
  });
